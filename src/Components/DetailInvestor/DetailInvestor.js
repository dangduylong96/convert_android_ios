...
import apiPostSendInvestor from '../../Api/apiPostSendInvestor';
...
class DetailInvestor extends Component{
	...
	submit(){
		...
		}else if(sourceCardBank==''){
			error=true;
			Toast.show({
	        	text: "Bạn chưa upload hình mặt trước thẻ ngân hàng",
	        	buttonText: "Okay",
	        	type: "danger",
	        	duration: 30000
      		})
	...
	render(){
		const { name, address, cmnd, placeOfCmnd, dayProviderCmnd, phone, numberBank, urlBeforeCMND, urlUnderCMND, urlCardBank, nameOfBank, editData, isSubmit, textBtnLogin}=this.state;
		return(
...
export default connect()(DetailInvestor)
