...
class InvestorSuccess extends Component{
	constructor(props){
		super(props);
		this.state={
            checkRender: true,
	...
	componentDidMount=async () =>{
        ...
        apiGetBorrowOfInvestor(accountDetail.idUser,accountDetail.emailUser,accountDetail.access_token,0)
        .then(res=>{
            this.setState({
                checkRender: false,
    ...
    removeBorrow=async (idBorrowOfInvestor) =>{
        let accountDetail=await AsyncStorage.getItem('@accountDetail');
        accountDetail=JSON.parse(accountDetail);
        fetch(`${URL.URL_BASE_SERVER}/prevent-borrow?userId=${accountDetail.idUser}&email=${accountDetail.emailUser}&accessToken=${accountDetail.access_token}&idBorrowOfInvestor=${idBorrowOfInvestor}`)
        .then(res=>{
        ...
        const listBorrow=dataBorrowInvestor.map(borrow=>(
            <View style={styles.headerContent} key={borrow.key}>
                ...
                <View style={styles.itemDivider}>
                    ...
                    <View style={styles.itemRight}>
                        <Text style={{fontWeight:'bold',fontSize: 15,color: '#ffcc00'}}><Icon type='SimpleLineIcons' name='cloud-upload' style={{color:'#ffcc00'}} /> Đang thực hiện</Text>
                    </View>
                </View>
                <View style={styles.itemDividerHeader}>
                    <TouchableOpacity onPress={()=>this.removeBorrow(borrow.key)}>
                        <Text style={styles.updateStatus}>Từ chối khoản vay</Text>
                    </TouchableOpacity>
                </View>
            </View>
        ))
...
export default connect()(InvestorSuccess)
